...
for line in range(len(Array)):
  ...
  #Algebra
    ...
    checker2 = y2 - y1
    Counter = 0
    if (pos(checker1) == pos(checker2)):
        if x2 < x1 and y2 < y1:
            for Move1 in range(pos(checker1)+1):
                MyArray2D[y1 - Counter][x1 - Counter] += 1
                Counter += 1
        elif x2 < x1 and y2 > y1:
            for Move2 in range(pos(checker1)+1):
                MyArray2D[y1 + Counter][x1 - Counter] += 1
                Counter += 1
        elif x2 > x1 and y2 > y1:
            for Move3 in range(pos(checker1)+1):
                MyArray2D[y1 + Counter][x1 + Counter] += 1
                Counter += 1
        elif x2 > x1 and y2 < y1:
            for Move1 in range(pos(checker1)+1):
